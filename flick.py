"""Canonical Flickr30k experiment driver for Bridged Clustering.

Runs the Flickr30k image-text or text-image sweep in the transductive or
inductive setting and writes `ami_x.npy`, `ami_y.npy`, `accuracy.npy`,
`mae.npy`, and `mse.npy` under `results/104_flick_2_*` or
`results/105_flick_rev_2_*`.
"""
...
def get_corpus():
    try:
        from bridged_clustering.datasets.flickr import load_flickr_corpus
    except ModuleNotFoundError as exc:
        raise ModuleNotFoundError(
            "Missing dependency while loading the Flickr30k corpus. Install packages from requirements.txt.",
        ) from exc
...
def run_experiment(
    ...
    *,
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "Missing dependency while running the Flickr30k experiment. Install packages from requirements.txt.",
        ) from exc
...
def run_reversed_experiment(
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "Missing dependency while running the reversed Flickr30k experiment. Install packages from requirements.txt.",
        ) from exc
...
def run_flickr_grid(
    *,
    ...
    prefix = "105_flick_rev_2" if reversed_direction else "104_flick_2"
    ...
    if len(eligible_clusters) < max(k_values):
        raise ValueError(
            f"Flickr30k needs at least {max(k_values)} eligible clusters of size {cluster_size}; "
            f"found {len(eligible_clusters)} after pruning.",
        )
...
def main() -> None:
    ...
    output_dir = run_flickr_grid(mode=args.mode, reversed_direction=args.reversed)
